...
const createPaymentIntent = catchAsync(async (req, res) => {
  ...
  const amount = donationCartItems.reduce(
    (total: number, item: any) => total + item.dollars * 100 + item.cents,
    0
  );
  ...
  await paymentService.updatePaymentIntent(paymentIntentId, {
    metadata: { donationIds: JSON.stringify(donationIds) },
  });
...
const sendDonationSuccessEmail = async (donationIds: string[]) => {
  ...
  const donations = await donationService.queryDonations(
    { id: { in: donationIds } },
...
const handleWebhookEvent = catchAsync(async (req, res) => {
  const { paymentStatus, paymentIntentId, donationIds } =
    await paymentService.handleWebhookEvent(req);
  var donationPaymentStatus: DonationPaymentStatus =
    DonationPaymentStatus.PENDING;
