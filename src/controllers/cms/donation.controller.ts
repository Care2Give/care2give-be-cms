...
const decryptDonations = (
  ...
  return donations.map(
    ({ donorNricA, donorNricB, campaignId, ...donation }) => {
      const nric =
        donorNricA && donorNricB
          ? `${encrypter.dencrypt(donorNricA)}${donorNricB}`
          : null;
      return {
        ...donation,
        nric,
...
const prepareExportRecords = (
  decryptedDonations: (Donation & {
    campaign?: Campaign;
    nric: string | null;
  })[]
) => {
...
const getCampaignNames = catchAsync(async (_, res) => {
  const campaigns = await donationService.getCampaignNames();
...
const exportDonationsToXlsx = catchAsync(async (req, res) => {
  ...
  const donations = await donationService.exportDonations({
    campaignIds,
    startDate,
    endDate,
  });
